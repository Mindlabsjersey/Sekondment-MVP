...
import { savePartnerProfile } from '../partner/actions';
import GlobalFieldset from './GlobalFieldset';

const INDUSTRIES = ['Agency', 'Consultancy', 'SaaS', 'Professional Services', 'Engineering',
  'Finance', 'Healthcare', 'Creative', 'Other'];
const SIZES = ['1–10', '11–50', '51–200', '201–500', '500+'];

export default function PartnerOnboardingForm({ defaultName }: { defaultName?: string }) {
  ...
  async function action(formData: FormData) {
    setPending(true);
    setError(null);
    formData.set('default_commission_pct', String(commission));
    const res = await savePartnerProfile(formData);
    ...
    <form action={action} className="space-y-5">
      <div>
        ...
        <input id="company_name" name="company_name" required defaultValue={defaultName}
          className="field" placeholder="Northpoint Studio" />
      </div>
      ...
      <div>
        <label className="label" htmlFor="description">About your company</label>
        <textarea id="description" name="description" rows={3} className="field resize-none"
          placeholder="What your company does and the kind of expertise you can deploy." />
      </div>
      ...
      <div>
        ...
        <p className="text-xs text-muted mt-1">
          Your default cut of an employee's net earnings (after the 15% platform fee). You can override this per employee.
        </p>
      </div>
